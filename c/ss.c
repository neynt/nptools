...
static void print_matrix(int *matrix, struct shape *shape, int i);
...
static void set_toggle(void) {
  ...
  for (i=0;i<max_token;i++) {
    if (i<=goal_token) new_order[i]=goal_token-i;
    else new_order[i]=goal_token+max_token-i;
  }
  for (i=0;i<ntokens;i++) matrix[i]=new_order[matrix[i]];
...
static void prepare_shapes(void) {
  ...
  for (i=nshapes-1;i>=0;i--) {
    ...
    shape->equal=0;
    for (j=i-1;j>=0;j--) {
      if (shapes[j].a.npoints!=shape->a.npoints) continue;
      for (k=shape->a.npoints-1;k>=0;k--) {
        if (shapes[j].a.points[k]!=shape->a.points[k]) break;
      }
      if (k<0) {
  ...
  for (i=0;i<ntokens;i++) toggles-=matrix[i];
...
static void find_sequence(void) {
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      --i;
      shape=shapes+i;
      if (shape->equal==0) seq=0;
      else seq=shape->equal->seq;
...
static void copy_shape_to_result(int *shape_graph, struct shape *shape, int **cache) {
  if (cache==0) cache=shape->cache+(shape->seq*shape->a.npoints);
...
static void print_matrix(int *matrix, struct shape *shape, int i) {
  ...
  if (matrix) memcpy(result_matrices[0],matrix,matrix_size);
  if (shape) copy_shape_to_result(shape_graphs[0],shape,shape->cache+i*shape->a.npoints);
  print_result_matrix(0,1);
}
...
void print_result(int print_matrix) {
  ...
  if (print_matrix) memcpy(result_matrix,start_matrix,sizeof(int) * ntokens);
  else memset(result_matrix,0,sizeof(int) * ntokens);
  for (m = 0; m < nshapes; ) {
    for (col=0;col<max_print_cols && m < nshapes;col++,m++) {
      for (i=nshapes-1;i>=0;i--) {
        shape=shapes+i;
        if (shape->a.nr==m) break;
      }
      n=shape->seq;
      printf("%2dx%2d=%3d",n%shape->a.x,n/shape->a.x,n);
      for (j=9;j<x*2+2;j++) putchar(' ');
